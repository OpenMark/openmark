 ...
 * in their XML. This version allows for credit (1 mark) to be given if an 
 * answer is partially correct at the final attempt. See the setScore() method below.
 * <p> 
 ...
 */
public abstract class SimpleQuestion3 extends StandardQuestion
{
  ...
  protected void setScore(boolean bRight,boolean bPass,int iAttempt) throws OmDeveloperException
  {
  	...
  	if(bPartial) getResults().setScore(1, Results.ATTEMPTS_PARTIALLYCORRECT);
  	else if(bPass) getResults().setScore(0, Results.ATTEMPTS_PASS);
  	else if(!bRight) getResults().setScore(0, Results.ATTEMPTS_WRONG);
