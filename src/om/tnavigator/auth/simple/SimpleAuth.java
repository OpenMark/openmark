 ...
 * 
 * Tables ------ simpleauth_users(username,password,[optional]email)
 * simpleauth_groups(username,groupname) Usernames and group names must contain
 * only alphanumerics.
 * 
 * Authentication -------------- Authentication is stored in a cookie
 * 'simpleauth': username:passwordhash The password hash is created by: 1) Take
 * user's password. 2) Append current magic number (as string). 3) Calculate
 * SHA-1 hash. The magic number is generated at random when the system starts
 * up. Note that I'm not certain how secure this method is, in particular it
 * might be better to change the number periodically... Also, Om is not intended
...
public class SimpleAuth implements Authentication {
	...
	/**
	 * Standard constructor. Creates database tables if they don't already
	 * exist.
	 * 
	 * @param ns
	 *            Servlet
	 * @throws SQLException
	 *             If any problem creating tables
	 */
	...
	protected void setUpDatabase() throws SQLException {
		...
		try {
			// Simple version that is supposed to work on all databases, hence
			// no
			// foreign key etc
			dat
					.update("CREATE TABLE simpleauth_users(username VARCHAR NOT NULL PRIMARY KEY, password VARCHAR NOT NULL,email VARCHAR)");
			dat
					.update("CREATE TABLE simpleauth_groups(username VARCHAR NOT NULL, groupname VARCHAR NOT NULL)");
			dat
					.update("CREATE INDEX simpleauth_groups_username ON simpleauth_groups(username)");
			return;
		} finally {
			dat.finish();
		}
	...
	public UncheckedUserDetails getUncheckedUserDetails(
		...
		if (cookie == null)
			...
			return new SimpleUncheckedUser(cookie, cookie.substring(0, cookie
					.indexOf(':')));
	}
	...
	private String hash(String password) throws IOException {
		...
		try {
			...
			throw new IOException(
					"Encryption system (java.security) not available: "
							+ e.getMessage());
		}
		try {
	...
	public UserDetails getUserDetails(HttpServletRequest request,
			HttpServletResponse response, boolean bRequireLogin)
			throws IOException {
		...
		if (details.getCookie() != null) {
			...
			try {
				dat = da.newTransaction();
				ResultSet rs = dat
						.query("SELECT password,email FROM simpleauth_users WHERE username="
								+ Strings.sqlQuote(details.getUsername()));
				if (rs.next()) {
					String password = rs.getString(1), email = rs.getString(2);
					if (details.getCookie().equals(
							details.getUsername() + ":" + hash(password))) {
						SimpleUser authenticated = new SimpleUser(details
								.getCookie(), details.getUsername(), email);
						rs = dat
								.query("SELECT groupname FROM simpleauth_groups WHERE username="
										+ Strings.sqlQuote(details
												.getUsername()));
						while (rs.next()) {
				...
				throw new IOException("Error authenticating user in database: "
						+ se.getMessage());
			} finally {
				if (dat != null)
					dat.finish();
	...
	public boolean handleRequest(String subPath, HttpServletRequest request,
		...
		if (request.getMethod().equals("POST")) {
			...
			try {
				ResultSet rs = dat
						.query("SELECT COUNT(*) FROM simpleauth_users WHERE username="
								+ Strings.sqlQuote(username)
								+ " AND password="
								+ Strings.sqlQuote(password));
				rs.next();
				if (rs.getInt(1) > 0) {
					// OK! Set cookie and redirect
					Cookie c = new Cookie(COOKIE, username + ":"
							+ hash(password));
					c.setPath("/");
					response.addCookie(c);
	...
	private String getLoginPageURL(HttpServletRequest request)
			throws IOException {
		// Get URL up to before webapp context, add the context back (doesn't
		// end
		// in /) then add in the path to login page
		String requestURL = request.getRequestURI();
		String redirectURL = requestURL.substring(0, requestURL.indexOf(request
				.getContextPath()))
	...
	public String sendMail(String username, String personID, String content,
		...
		try {
			dat = da.newTransaction();
			ResultSet rs = dat
					.query("SELECT email FROM simpleauth_users WHERE username="
							+ Strings.sqlQuote(username));
			if (rs.next()) {
				String address = rs.getString(1);
				Mail.send(mailFrom, mailFrom, new String[] { address }, null,
						subjectAndMessage[0], subjectAndMessage[1], null);
				return "";
			} else {
				throw new IOException("Can't find email address for user");
			}
		} catch (SQLException se) {
			throw new IOException("Error looking up user email address: "
					+ se.getMessage());
